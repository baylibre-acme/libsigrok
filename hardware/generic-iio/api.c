...
static struct sr_dev_driver *di = &generic_iio_driver_info;
					    ...
					    int device_id)
{
	...
	chgp->pchans = g_malloc0(sizeof(struct channel_priv) * nb_chan);
	if (!chgp->pchans) {
		sr_err("Failed to allocate private channels group structure");
		return;
	}
	...
	ret = iio_device_attr_read(chgp->iio_dev, "in_sampling_frequency",
				   temp, 16);
	if (ret) {
		sr_info("%s: in_sampling_frequency: %s", chg->name, temp);
	...
	for (i = 0; i < nb_chan; i++) {
		...
		chan = sr_channel_new(i, SR_CHANNEL_ANALOG, TRUE, probename);
		chanp->chan = chan;

		chg->channels = g_slist_append(chg->channels, chan);
		sdi->channels = g_slist_append(sdi->channels, chan);
		chgp->channel_privs = g_slist_append(chgp->channel_privs, chanp);
	}
...
static GSList *scan(GSList *options)
{
	...
	sdi = sr_dev_inst_new(0, SR_ST_INACTIVE, "Generic IIO",
				iio_ctx_name, iio_ctx_version);
	sdi->driver = di;
...
static int dev_clear(void)
{
	...
	for (i = dev_list(); i ; i = i->next) {
		sdi = i->data;

		sr_dbg("Freeing dev instance %d", sdi->index);

		for (l = sdi->channel_groups; l; l = l->next) {
			chg = l->data;
			chgp = chg->priv;

			sr_dbg("Freeing group %s private data", chg->name);

			g_free(chgp->pchans);

			sr_dbg("Freeing group %s", chg->name);
		...
		for (l = sdi->channels; l; l = l->next) {
			...
			sr_dbg("Freeing channel %s", ch->name);
...
static int config_get(int id, GVariant **data,
		...
		case SR_CONF_SAMPLERATE: {
			...
			for (chgl = sdi->channel_groups;
				...
				if (srate && chgp->sample_rate != srate) {
					sr_warn("Sampling rate is different among devices, report 1Hz");
					srate = 1;
					break;
				}
		...
		case SR_CONF_BUFFERSIZE: {
			...
			for (chgl = sdi->channel_groups;
				...
				if (bufsize && chgp->buffer_size != bufsize) {
					sr_warn("Buffer size among devices, report min");
					bufsize = MIN(bufsize, chgp->buffer_size);
					break;
				}
...
static int config_set(int id, GVariant *data,
		      const struct sr_dev_inst *sdi,
		      const struct sr_channel_group *chgs)
{
	struct channel_group_priv *chgp = NULL;
...
static int config_list(int info_id, GVariant **data,
		       const struct sr_dev_inst *sdi,
...
static int dev_acquisition_start(const struct sr_dev_inst *sdi, void *cb_data)
{
	...
	for (chgl = sdi->channel_groups; chgl; chgl = chgl->next) {
		...
		for (chpl = chgp->channel_privs ; chpl ; chpl = chpl->next) {
			struct channel_priv *chanp = chpl->data;
			struct sr_channel *chan = chanp->chan;
		...
		sr_dbg("%d samples big IIO buffer allocated",
			chgp->buffer_size);
		...
		if (fd < 0) {
			...
			sr_session_source_add(chgp->poll_thread.pipe_fds[0],
					      POLLIN, 100,
					      gen_iio_receive_data,
					      (void *)chg);
		}
		else
			sr_session_source_add(fd, POLLIN, 100,
					      gen_iio_receive_data,
					      (void *)chg);
...
static int dev_acquisition_stop(struct sr_dev_inst *sdi, void *cb_data)
{
	...
	for (chgl = sdi->channel_groups; chgl; chgl = chgl->next) {
		...
		if (chgp->poll_thread.enabled) {
			sr_session_source_remove(chgp->poll_thread.pipe_fds[0]);
			...
			sr_session_source_remove(iio_buffer_get_poll_fd(iiobuf));
